...
default implementation. For Omniverse Kit hosts, use
:class:`warp.utils.LoggerKit`. Frameworks can supply custom loggers via
...
def log_warning(message: str, category=None, stacklevel=1, once=False) -> None:
    ...
    # +1: skip log_warning() so the warning points at the caller
    _active_logger.warning(message, category=category, stacklevel=stacklevel + 1)
